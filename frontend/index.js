...
window.onload = async () => {
  ...
  document.getElementById('test-date').innerHTML = date
}
...
document.querySelector('form').onsubmit = async (event) => {
  ...
  try {
    await contract.deposit(donation.value)
  } catch (e) {
    alert(
...
async function getAndShowContractData() {
  // Load last 10 donations
  // let contractData = await contract.contractData()

  document.getElementById('target_amount').innerHTML = await contract.getTargetAmount()
...
document.getElementById('change-beneficiary-button').onclick = async (event) => {
  event.preventDefault()
  // console.log('clicked');
  let newAccount = 'near-l1.testnet'
  await contract.changeBeneficiary(newAccount)
}
